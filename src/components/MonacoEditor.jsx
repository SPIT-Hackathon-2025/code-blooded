...
function MonacoEditor({ socketRef, roomId, onCodeChange }) {
  ...
  useEffect(() => {
    // Step 1: Simulated retrieval (string from DB)
    const retrievedCode = "'print(\"hello world\")\\r\\nprint(\"hi world\")'";

    // Step 2: Remove extra quotes
    let cleanedCode = retrievedCode.slice(1, -1);

    // Step 3: Decode escape sequences
    cleanedCode = cleanedCode.replace(/\\r\\n/g, "\n").replace(/\\"/g, '"');

    // Step 4: Set the formatted code into Monaco Editor
    setValue(cleanedCode);
  }, []);
